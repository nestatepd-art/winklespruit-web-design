...
const Auth = () => {
  ...
  useSEO({
    title: 'Client Zone Login | Native Digital Media',
    description: 'Secure login to access your invoices, project status, and account.',
    canonical: '/auth',
  });
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-12">
      <Link to="/" className="mb-8">
        <img src={logo} alt="Native Digital Media" className="h-14 w-auto" />
      </Link>

      <Card className="w-full max-w-md p-8 card-gradient border-border">
        <div className="text-center mb-6">
          <h1 className="font-heading text-2xl font-bold mb-2">Client Zone</h1>
          <p className="text-muted-foreground text-sm">Manage invoices and track your project</p>
        </div>
          ...
          <Tabs defaultValue="signin">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="space-y-4">
                <div>
                  <Label htmlFor="signin-email"><Mail className="w-3.5 h-3.5 inline mr-1" />Email</Label>
                  <Input id="signin-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div>
                  <Label htmlFor="signin-password"><Lock className="w-3.5 h-3.5 inline mr-1" />Password</Label>
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="space-y-4">
                <div>
                  <Label htmlFor="signup-name"><User className="w-3.5 h-3.5 inline mr-1" />Full name</Label>
                  <Input id="signup-name" required value={fullName} onChange={(e) => setFullName(e.target.value)} />
                </div>
                <div>
                  <Label htmlFor="signup-email"><Mail className="w-3.5 h-3.5 inline mr-1" />Email</Label>
                  <Input id="signup-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div>
                  <Label htmlFor="signup-password"><Lock className="w-3.5 h-3.5 inline mr-1" />Password (min 8 chars)</Label>
